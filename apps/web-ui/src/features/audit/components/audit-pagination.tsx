/**
 * Pagination controls for the audit explorer.
 *
 * Renders page navigation with previous/next buttons, page numbers,
 * current position indicator, and a page-size selector. Follows the
 * same visual pattern as the task board pagination.
 *
...
/**
 * Renders pagination controls for the audit event list.
 *
 * Shows the current range of items (e.g. "1-20 of 156 events"), page
 * buttons with smart ellipsis, and a page-size selector. Hidden when
 * there are no events to paginate.
 */
export function AuditPagination({
  ...
  return (
    ...
    >
      {/* Items range display */}
      <div className="text-sm text-muted-foreground" data-testid="audit-pagination-info">
        ...
        <div className="flex gap-1">
          {PAGE_SIZE_OPTIONS.map((size) => (
            <Button
              ...
              data-testid={`audit-page-size-${size}`}
            >
              {size}
            </Button>
          ))}
